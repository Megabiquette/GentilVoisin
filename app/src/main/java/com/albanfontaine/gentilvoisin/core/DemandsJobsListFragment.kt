...
class DemandsJobsListFragment : BaseJobsListFragment() {
    ...
    override fun getJobs() {
        jobList = ArrayList()
        JobRepository.getJobsByType(userCity, "demand").addOnCompleteListener { task ->
        ...
        val job3 = Job("3", "KYV4tvJBIBcuZtc4O3FzaSitNTq1", userCity,"Ménage",
            "demand", "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo", Date(), true)
        jobList.apply {
            add(job1)
            add(job2)
            add(job3)
